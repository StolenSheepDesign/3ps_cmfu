                ...
                foreach ($tagArray["attr"] as $attr => $attrValue) {
                    if (isset($tagArray["variableFeed"])) {
                        foreach ($tagArray["variableFeed"] as $item) {
                            ...
                            if ($attr == "style") { // Style array
                                foreach ($attrValue as &$ruleValue) {
                                    $ruleValue = self::replacePlaceholders($item, $ruleValue, $optionValue);
                                }
                            } elseif ($attr == "class") { // Class array
                                foreach ($attrValue as &$classes) {
                                    $classes = self::replacePlaceholders($item, $classes, $optionValue);
                                }
                            } else {
                    ...
                    if ($attr == "style") { // Style
                        ...
                        foreach ($attrValue as $class) {
                            $finalTags[$tag]["attr"][$attr][] = $class;
                        }
        ...
        foreach ($finalTags as $tag => $tagArray) {
            ...
            foreach ($tagArray["attr"] as $attr => $attrValue) {
                ...
                if ($attr == "style") {
                    ...
                    //var_dump($attrValue);
                    $compiledAttrValue .= implode(" ", array_values($attrValue));
                } else {
